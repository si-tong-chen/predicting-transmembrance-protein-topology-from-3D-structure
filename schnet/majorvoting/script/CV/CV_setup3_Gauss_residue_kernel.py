...
setup_train_data, setup_train_label, setup_train_batchname, setup_train_dismatch_index_pred, setup_train_dismatch_index_type, setup_train_real_node_label, setup_train_df, \
  setup_val_data, setup_val_label, setup_val_batchname, setup_val_dismatch_index_pred, setup_val_dismatch_index_type, setup_val_real_node_label, setup_val_df,\
  setup_test_data, setup_test_label, setup_test_batchname, setup_test_dismatch_index_pred, setup_test_dismatch_index_type, setup_test_real_node_label, setup_test_df = make_splits(cv0_lis, cv1_lis, cv2_lis, cv3_lis, cv4_lis, "setup 3")
...
wandb.init(
    ...
    group="CV setup 3, major voting, schnet", # 对比实验分组
    name= "CV setup 3 with batchsize 1, early stopping and exp learning schedule", #实验的名字
...
torch.save(model.state_dict(), '/work3/s194408/Project/result/schnet/CV_setup3_Gauss_result.pth')
...
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup3_node_acc_results_Gauss.csv", node_acc_results, delimiter=',', comments="", fmt='%s')

loss_results = np.concatenate([ [np.array(epochs)], [np.array(epoch_loss_record_train)], [np.array(epoch_loss_record_val)] ])
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup3_loss_results_Gauss.csv", loss_results, delimiter=',', comments="", fmt='%s')
