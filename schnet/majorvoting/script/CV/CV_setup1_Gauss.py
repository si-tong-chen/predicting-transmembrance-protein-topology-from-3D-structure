...
wandb.init(
    ...
    name= "CV setup 1 with batchsize 1, 6 layers, 5 neigbors", #实验的名字
...
for epoch in range(total_epochs):
    ...
    for i, data in enumerate(setup1_train_data):  
        ...
        #prediction_Gauss = torch.tensor(gaussian_filter1d(prediction.cpu().detach().numpy(), 1, radius=3), requires_grad=True)
...
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup1_node_acc_results_Gauss.csv", node_acc_results, delimiter=',', comments="", fmt='%s')

loss_results = np.concatenate([ [np.array(epochs)], [np.array(epoch_loss_record_train)], [np.array(epoch_loss_record_val)] ])
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup1_loss_results_Gauss.csv", loss_results, delimiter=',', comments="", fmt='%s')
