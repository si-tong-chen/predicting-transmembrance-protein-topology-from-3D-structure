...
wandb.init(
    ...
    name= "CV setup 1 with batchsize 1, 6 layers, 32 neigbors, reduced Gaussian kernel (21)", #实验的名字
...
for epoch in range(total_epochs):
    ...
    for i, data in enumerate(setup1_train_data):  
        ...
        predicted = F.pad(predicted, (10, 10), mode='reflect')
...
torch.save(model.state_dict(), '/work3/s194408/Project/result/CV_setup1_5_neighbors_Gauss_big_kernel_3.pth')
...
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup1_node_acc_results_Gauss_big_kernel_3.csv", node_acc_results, delimiter=',', comments="", fmt='%s')

loss_results = np.concatenate([ [np.array(epochs)], [np.array(epoch_loss_record_train)], [np.array(epoch_loss_record_val)] ])
np.savetxt("/work3/s194408/Project/result/schnet/CVsetup1_loss_results_Gauss_big_kernel_3.csv", loss_results, delimiter=',', comments="", fmt='%s')
