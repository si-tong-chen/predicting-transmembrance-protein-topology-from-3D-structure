...
class GCPEmbedding(nn.Module):
    ...
    def forward(
        ...
        if self.atom_embedding is not None:
            node_rep = ScalarVector(self.atom_embedding(batch.h.unsqueeze(1).to(torch.float32)), batch.chi.to(torch.float32))
        else:
            node_rep = ScalarVector(batch.h.unsqueeze(1).to(torch.float32), batch.chi.to(torch.float32))
        ...
        edge_rep = ScalarVector(
            torch.cat(
                (edge_rep.scalar.unsqueeze(1), self.radial_embedding(edge_lengths)), dim=-1
            ),
            edge_rep.vector,
        )
